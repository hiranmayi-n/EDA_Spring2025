...
```{r project.setup, echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE, results='hide'}
#Import libraries
library(tidyverse)
library(lubridate)
library(here)
...
```{r data.summary, echo=FALSE, message=FALSE, warning=FALSE, results='markup'}
...
# Produce a table of mean PM2.5 concentrations
kable(
  summary_PM25,
...
# Produce a table of mean Ozone concentrations
kable(
  summary_Ozone,
  col.names = c("County", "$\\mu g/m^3$"),
  caption = "Mean Ozone"
...
```{r ozone_boxplot_2018, echo=FALSE, fig.align='left', fig.height=3, fig.width=5, fig.cap="Ozone Levels by Month (2018)"}

library(ggplot2)

ggplot(subset(epa_data, Year == 2018), aes(x = factor(Month), y = Ozone)) +
  geom_boxplot() +
  labs(x = "Month", y = "Ozone (ppm)") +
  theme_minimal()

```

```{r ozone_boxplot_2019, echo=FALSE, fig.align='left', fig.height=3, fig.width=5, fig.cap="Ozone Levels by Month (2019)"}

library(ggplot2)

ggplot(subset(epa_data, Year == 2019), aes(x = factor(Month), y = Ozone)) +
  geom_boxplot() +
  labs(x = "Month", y = "Ozone (ppm)") +
  theme_minimal()
